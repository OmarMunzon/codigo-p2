...
// importar imagen boceto
document.getElementById('imageUp').addEventListener('change', function () {
    const file = this.files[0];
    if (file) {
        sendImageToServer("/detectar", file);        
    }
});
...
function sendImageToServer(route, file) {
...
function crearComponente(draggedType,x,y){
    let element;
    switch (draggedType) {
        case 'button':
            element = new joint.shapes.standard.Rectangle();
